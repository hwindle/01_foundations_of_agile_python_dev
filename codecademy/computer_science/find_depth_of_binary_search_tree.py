...
def build_bst(bst_list):
    if len(bst_list) == 0:
        return None

    mid_idx = len(bst_list) // 2
    mid_val = bst_list[mid_idx]

    tree_node = {'data': mid_val}
    tree_node['left_child'] = build_bst(bst_list[:mid_idx])
    tree_node['right_child'] = build_bst(bst_list[mid_idx + 1 :])
    return tree_node
